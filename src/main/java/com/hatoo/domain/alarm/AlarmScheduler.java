...
public class AlarmScheduler {
    ...
    // ──────────────────────────────────────────
    // 1. 할일 시작 알림 - 매일 오전 8시
    //    dueTo 날짜가 오늘인 미완료 할일 담당자에게 전송
    // ──────────────────────────────────────────
    @Scheduled(cron = "0 0 8 * * *")
    @Transactional
    public void sendTaskStartAlarm() {
        String today = LocalDate.now().toString();
        log.info("[AlarmScheduler] 할일 시작 알림 - 기준일(dueTo): {}", today);

        List<Task> tasks = taskRepository.findByDueToStartingWithAndFinishedFalseAndStartAlarmSentFalse(today);
        tasks.forEach(task -> {
        ...
        log.info("[AlarmScheduler] 할일 시작 알림 발송 완료 - {}건", tasks.size());
    }
    ...
    public void sendTaskDeadlineAlarm() {
        ...
        tasks.forEach(task -> {
            ...
            // notifyAt이 이미 지났고 (현재 시각 >= notifyAt),
            // 너무 오래 지나지 않은 경우 (최대 10분 이내)에만 발송
